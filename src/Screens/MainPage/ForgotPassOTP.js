import { StyleSheet, Text, View, StatusBar, Image, TouchableOpacity, ActivityIndicator, Alert, BackHandler } from 'react-native'
  ...
  const [showPass2, setShowPass2] = useState(false);
    ...
    const [confirmnewpassword, setConfirmnewPassword] = useState('');
      ...
      const handleForgotPass = () => {
       ...
       {
        alert("newpassword and confirm password should be same")
        setLoading(false)
        }
    
    else {
        setLoading(true)
        fetch('http://coaching.rootstechnology.in/api/ForgetPassword', {
            method: 'post',
            headers: {
    ...
    return (
        <View>
            <View style={[styles.container]}>
                ...
                <View style={{ marginTop: 0, marginBottom: 5, marginLeft: 0,alignItems:'center' }}>
                    <Text style={{ fontSize: 15, fontWeight: 'bold', color: '#111' }}> Phone:000000000</Text>
                </View>

                <TextInput style={[styles.searchbar]} keyboardType='numeric' maxLength={4} placeholder="OTP"
                    onChangeText={(text) => {
                        setOTP(text)
                    }} />
                    <InputField
